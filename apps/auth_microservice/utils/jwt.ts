...
export interface TokenPayload extends jwt.JwtPayload {
  ...
  email: string;
  jti: string;
  // exp добавляется библиотекой jwt автоматически, но мы указываем его явно для TS
  exp?: number; 
}
...
export const generateAccessToken = (payload: Omit<TokenPayload, 'jti' | 'exp' | 'iat'>) => {
  ...
  // Fix:
  // 1. ACCESS_SECRET приводим к string, так как process.env может быть undefined.
  // 2. Объект опций приводим к jwt.SignOptions, чтобы TypeScript корректно выбрал перегрузку функции.
