...
export default function FriendsPage() {
  ...
  const handleRemoveFriend = async (profileId: string, username: string) => {
    if (!confirm(t('friends.removeFriendConfirm', { username }))) return;
    try {
  ...
  const handleAcceptRequest = async (username: string) => {
    try {
      ...
      alert(t('friends.failedAcceptRequest'));
    }
  };
  ...
  const handleRejectRequest = async (username: string) => {
    try {
      ...
      alert(t('friends.failedRejectRequest'));
    }
  };
    ...
    if (users.length === 0) {
      return (
        <div className="text-center py-12">
          ...
          <p style={{ color: 'var(--text-muted)' }}>{t('friends.noUsers')}</p>
        </div>
      );
    }
    ...
    if (requests.length === 0) {
      return (
        <div className="text-center py-12">
          ...
          <p style={{ color: 'var(--text-muted)' }}>{t('friends.noPendingRequests')}</p>
        </div>
      );
    }
